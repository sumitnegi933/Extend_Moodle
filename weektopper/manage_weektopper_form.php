...
class manage_weektopper_form extends moodleform {
    ...
    public function validation($data, $files) {
       ...
       /* $current_available_from = $data['available_from'];
        ...
        $sql = "SELECT * FROM {block_weektopper} WHERE quiz =:quiz ";
        if ($DB->get_records_sql($sql, array('quiz'=>$quizitem))) {
            $errors['courselist'] = get_string('quizalreadyexist', 'block_weektopper');
        }
        /*if ($data['available_from'] >= $data['available_to']) {
            $errors['available_from'] = get_string('dateerror', 'block_weektopper');
        }*/
        if (empty($data['courselist'])) {
            $errors['courselist'] = get_string('selectcourseerror', 'block_weektopper');
        }

        if (empty($quizitem)) {
            $errors['courselist'] = get_string('selectquizerror', 'block_weektopper');
        }
